  ...
  if (cookiesManager.isCookiesAllowed()) {
    ...
    if (credentials) {
      try {
        ...
        // 清除可能已损坏的凭证
        cookiesManager.clearSpecificCookies(['remembered_credentials'])
      }
    }
  }
})
    ...
    if (result.error) {
      ...
      if (cookiesManager.isCookiesAllowed()) {
        if (loginForm.rememberMe) {
          ...
          cookiesManager.setCookie('remembered_credentials', encryptedData, { expires: 30 })
        } else {
          // 清除已保存的凭证
          cookiesManager.clearSpecificCookies(['remembered_credentials'])
        }
        ...
        // 用户拒绝了cookies，不存储凭证
        console.warn('Cannot store credentials: user has rejected cookies')
      }
...
const validateForm = () => {
  ...
  if (signUpForm.password !== signUpForm.password_confirm) {
    loginMessage('The passwords entered twice do not match.', 'error')
    isValid = false;
    return isValid;
  }
